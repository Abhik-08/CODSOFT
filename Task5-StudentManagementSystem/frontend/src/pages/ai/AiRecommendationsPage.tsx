...
export default function AiRecommendationsPage() {
  ...
  const forecastData = [
    ...
    { name: 'Sem 2', block: 8.78, forecast: 8.78 },
    { name: 'Sem 3', block: 8.95, forecast: 8.95 },
    { name: 'Sem 4', block: 9.13, forecast: 9.13 },
    { name: 'Sem 5 (F)', block: null, forecast: 9.3 },
    { name: 'Sem 6 (F)', block: null, forecast: 9.55 },
  ]
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      let replyText = "I've analyzed the student database queries. The general registry records indicate standard attendance ratios at 94.2% and an average CGPA benchmark of 9.12."
      ...
      if (lowerInput.includes('risk') || lowerInput.includes('warning') || lowerInput.includes('at risk')) {
        ...
        replyText = `The current cohort average CGPA is 9.12. Computer Science has the highest CGPA averages at 9.55, while Mechanical Engineering requires immediate tutorial counseling.`
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-vault-fg to-vault-fg/75 bg-clip-text text-transparent flex items-center gap-2">
            <Brain className="text-vault-accent" />
            <span>Academic Intelligence Center</span>
          </h2>
          <p className="text-slate-400 mt-1 text-sm">Predictive forecasts, cohort risk detection, and automated student advisories.</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 bg-vault-accent/15 border border-vault-accent/20 rounded-xl text-xs font-bold text-vault-accent font-mono shadow-sm">
          <Zap size={13} className="animate-pulse" />
          <span>PREDICTIVE COGNITIVE ENGINE ACTIVE</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="vault-glass p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            ...
            <span className="text-[9px] text-slate-400">Scan registry boundaries</span>
          </div>
        </div>

        {/* KPI 2: Placement Ready Projections */}
        <div className="vault-glass p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            ...
            <span className="text-[9px] font-semibold text-emerald-500 flex items-center gap-0.5 mt-0.5">
        ...
        <div className="vault-glass p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            ...
            <span className="text-[9px] text-slate-400">CSE stable; Mech Alert threshold</span>
          </div>
        </div>

        {/* KPI 4: Academic Trend Monitoring */}
        <div className="vault-glass p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-r from-vault-accent/5 to-vault-accent/0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
          ...
          <div className="h-60 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastData} margin={{ left: -25, bottom: 0, right: 10 }}>
                ...
                {/* Area chart representing actual path up to Sem 4 */}
                <Area 
                  type="monotone" 
                  dataKey="actual" 
                ...
                {/* Line chart overlay showing forecasted path continuing to Sem 6 */}
                <Line 
                  type="monotone" 
                  dataKey="forecast" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 vault-glass p-5 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm text-left flex flex-col justify-between h-[450px]">
          ...
          <div className="flex-1 overflow-y-auto mt-4 pr-1 space-y-2.5">
            {atRiskStudents.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-500 font-mono text-xs">
                No active cohort risk parameters triggered. All models optimal.
              </div>
            ) : (
              atRiskStudents.map((student) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-full bg-slate-200 dark:bg-white/5 border border-slate-300 dark:border-white/10 flex items-center justify-center font-bold text-xs">
                      {student.firstName[0]}
                    </div>
                    <div>
                      <p className="text-xs font-bold text-slate-800 dark:text-white">{student.firstName} {student.lastName}</p>
                      <p className="text-[9.5px] text-slate-400">{student.department} • Sem {student.semester}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center flex-wrap gap-2.5">
                    ...
                    {/* Risk Badge */}
                    <span className={`text-[8.5px] font-extrabold px-1.5 py-0.5 rounded uppercase font-mono border ${
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-vault-accent hover:bg-vault-accent/90 text-white rounded-lg font-bold text-[9.5px] shadow-sm transition-all active:scale-[0.98] cursor-pointer"
                  >
                    Send Advisory
                  </button>
                </div>
        ...
        <div className="lg:col-span-5 vault-glass p-5 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm text-left flex flex-col h-[450px] justify-between">
          <div className="space-y-1">
            <h3 className="text-sm font-extrabold text-slate-800 dark:text-white uppercase tracking-wider flex items-center gap-2">
              <Brain size={15} className="text-vault-accent" />
              <span>EduVault Intelligent Copilot</span>
            </h3>
            <p className="text-[10px] text-slate-400">Ask metrics questions relating to risk trends, department GPAs, or attendance anomalies.</p>
          </div>

          {/* Dialog Container */}
          <div className="flex-1 bg-slate-50/50 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 rounded-xl p-4 overflow-y-auto my-4 space-y-3.5 pr-2">
            {messages.map((m) => (
              <div 
                key={m.id} 
              ...
              >
                ...
                <div className={`p-2.5 rounded-xl max-w-[85%] text-xs leading-normal font-medium ${
                  m.sender === 'user' 
                    ? 'bg-vault-cyan text-white rounded-tr-none' 
          ...
          <div className="flex gap-2 items-center border border-slate-200 dark:border-white/5 p-1.5 rounded-xl bg-slate-50/50 dark:bg-white/5">
            <input
              ...
              className="flex-1 bg-transparent border-0 text-xs px-2.5 py-1.5 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none font-semibold"
            />
            <button
              onClick={handleSendMessage}
              className="p-2 bg-vault-accent hover:bg-vault-accent/90 text-white rounded-lg transition-all active:scale-95 cursor-pointer flex items-center justify-center"
      ...
      {/* Advisory Dispatch Modal overlay */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            <button 
              ...
              className="absolute top-4 right-4 text-slate-400 hover:text-vault-fg transition-colors"
            ...
            <p className="text-[10px] text-slate-400 mt-1">Configure and send a dynamic educational advisor intervention alert.</p>

            <form onSubmit={handleSendAdvisory} className="mt-4 space-y-4">
              <div className="p-3 bg-slate-50/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 rounded-xl text-xs space-y-1">
                <p className="font-bold text-slate-800 dark:text-white">Recipient Details</p>
                <p className="text-slate-400">Student: {advisoryModalStudent.firstName} {advisoryModalStudent.lastName}</p>
                <p className="text-slate-400">GPA: {advisoryModalStudent.gpa.toFixed(2)} • Attd: {advisoryModalStudent.attendanceRate}%</p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 rounded-xl focus:outline-none focus:border-vault-accent transition-colors"
                  required
                />
              </div>
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button
                  type="button"
                  onClick={() => setAdvisoryModalStudent(null)}
                  className="px-4 py-2 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-bold transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-vault-accent to-vault-cyan text-white rounded-xl text-xs font-bold shadow-md shadow-vault-accent/15 cursor-pointer flex items-center gap-1.5 transition-all"
                >
                  {advisorySentStatus ? (
                    <>
